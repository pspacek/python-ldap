% $Id: ldap.tex,v 1.39 2008/03/26 12:22:55 stroeder Exp $
...
\subsection{Constants \label{ldap-constants}}
...
\subsubsection{Options \label{ldap-options}}
...
\begin{datadesc}{OPT_DEREF}
  Specifies how alias derefencing is done within
...
\begin{datadesc}{OPT_DIAGNOSTIC_MESSAGE}
\end{datadesc}
...
\subsubsection{DN format flags \label{ldap-dn-flags}}

This constants are used for DN-parsing functions found in
sub-module \refmodule{ldap.dn}.

\begin{seealso}
  \manpage{ldap_str2dn}{3}
\end{seealso}

\begin{datadesc}{DN_FORMAT_LDAP}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_LDAPV3}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_LDAPV2}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_DCE}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_UFN}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_AD_CANONICAL}
\end{datadesc}

\begin{datadesc}{DN_FORMAT_MASK}
\end{datadesc}

\begin{datadesc}{DN_PRETTY}
\end{datadesc}

\begin{datadesc}{DN_SKIP}
\end{datadesc}

\begin{datadesc}{DN_P_NOLEADTRAILSPACES}
\end{datadesc}

\begin{datadesc}{DN_P_NOSPACEAFTERRDN}
\end{datadesc}

\begin{datadesc}{DN_PEDANTIC}
\end{datadesc}
...
\subsection{Exceptions \label{ldap-exceptions}}
...
The \refmodule{ldap.controls} module can be used for constructing and
...
The \var{msgid} argument should be the message ID of an outstanding LDAP
operation as returned by the asynchronous methods
\method{search()}, \method{modify()}, etc. 
The caller can expect that the result of an abandoned operation will not be
returned from a future call to \method{result()}.
\end{methoddesc}
...
\begin{methoddesc}[LDAPObject]{add}{dn, modlist} % -> int
     \methodline{add_s}{dn, modlist} % -> None
     \methodline{add_ext}{dn, modlist \optional{, serverctrls=\constant{None} \optional{, clientctrls=\constant{None}}}} % -> int
     \methodline{add_ext_s}{dn, modlist \optional{, serverctrls=\constant{None} \optional{, clientctrls=\constant{None}}}} % -> None
...
The asynchronous methods \method{add()} and \method{add_ext()}
...
\begin{methoddesc}[LDAPObject]{modify}{ dn, modlist } % -> int
     ...
     \methodline{modify_ext}{ dn, modlist \optional{, serverctrls=\constant{None} \optional{, clientctrls=\constant{None}}}} % -> int
     \methodline{modify_ext_s}{ dn, modlist \optional{, serverctrls=\constant{None} \optional{, clientctrls=\constant{None}}}} % -> None
...
The asynchronous methods \method{modify()} and \method{modify_ext()}
return the message ID of the initiated request.
...
\begin{methoddesc}[LDAPObject]{start_tls_s}{} % -> None    
    ...
    set to \constant{VERSION3} (which it is by default) before calling this method.
    If TLS could not be started an exception will be raised.
  \begin{seealso}
  \seerfc{2830}{Lightweight Directory Access Protocol (v3): Extension for Transport Layer Security}{}
  \end{seealso}
\end{methoddesc}
...
These methodsare all synchronous in nature.
\end{methoddesc}
