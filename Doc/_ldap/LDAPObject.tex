...
% $Id: LDAPObject.tex,v 1.4 2000/08/20 14:14:27 leonard Exp $
...
The \var{msgid} parameter is the integer identifier previously returned by the
...
\var{msgid} should be passed as the constant \constant{RES_ANY}.

The \var{all}
parameter has meaning only for \method{search} responses, and is used to select 
...
If \var{all} is \constant{0}, one response at a time is returned, with 
...
If \var{timeout} is negative (the default), the method will wait indefinitely.
...
except that many search parameters are pulled out of the \var{url}. 
